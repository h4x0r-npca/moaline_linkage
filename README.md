...
- 물리 COM 포트 선택 및 포트별 고급설정
- 실행만으로 COM 포트를 열지 않도록 안전 대기 상태로 시작
...
`moa_linkageSM.exe`는 선택된 물리 COM 포트를 감시하고, 영수증 한 장 단위로 `.LOG` 파일을 생성합니다.

중요: 일반 `pyserial` 방식의 직접 감시는 COM 포트를 독점으로 열 수 있습니다. 그래서 프로그램 시작, 포트 선택, 설정 저장만으로는 포트를 열지 않습니다. `테스트 감시 시작`은 POS/프린터가 해당 포트를 사용하지 않는 테스트 환경에서만 사용하세요. 운영 매장의 무중단 후킹은 무료/오픈소스 드라이버 PoC가 성공한 뒤 별도로 적용합니다.
